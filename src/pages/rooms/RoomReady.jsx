    ...
    const checkRoomAvailability = async () => {
      try {
        ...
        if (response.isAvailable) {
          ...
          const gameUrl = `http://game.example.com/rooms?roomId=${response.roomId}&clientId=${response.clientId}`;
          window.location.href = gameUrl; // 브라우저에서 외부 URL로 이동
        } else {
          alert(response.message || "방 입장이 불가능합니다.");
          navigate("/rooms"); // 방 목록으로 리다이렉트
        }
      } catch (error) {
        console.error("방 입장 확인 중 오류 발생:", error);
        alert("방 입장 여부를 확인할 수 없습니다. 다시 시도해주세요.");
        navigate("/rooms"); // 오류 발생 시 방 목록으로 리다이렉트
      }
    };
